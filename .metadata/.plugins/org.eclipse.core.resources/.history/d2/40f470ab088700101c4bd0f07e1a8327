...
public class Exam01 {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < comNum.length; i++) {		    
		    do {
		        comNum[i] = (int)(Math.random()*10); // 0~9 사이 숫자
		        // 이전 인덱스들과 비교
		        for (int j = 0; j < i; j++) {
		...
		String[] tokens = input.trim().split("\\s+"); // 공백 기준으로 나누기

		for (int i = 0; i < tokens.length; i++) {
		    userNum[i] = Integer.parseInt(tokens[i]); // 문자열 → 정수 변환
		}
		...
		int out = comNum.length - strike - ball;
